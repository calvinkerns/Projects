...
export class GaussianUpdater {
    ...
    createSortKey(tileIndex, depth) {
        // Don't invert depth - smaller depth values (closer to camera) should come first because of the front to back rendering
        const depthBits = Math.floor(depth * 0xFFFFFFFF);
    ...
    if (clipSpace[2] >= -1.0 && clipSpace[2] <= 1.0) {
        const w = clipSpace[3];  // Use w component for perspective divide
        ...
        const depth = clipSpace[2] / w;  // Normalized device coordinates depth
    ...
    computeCovariance(scales, rotation) {
